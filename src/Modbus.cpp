...
DiscreteInputType Modbus::readDiscreteInput(uint8_t index) const
{
...
CoilType Modbus::readCoil(uint8_t index) const
{
...
InputRegisterType Modbus::readInputRegister(uint8_t index) const
{
...
HoldingRegisterType Modbus::readHoldingRegister(uint8_t index) const
{
...
void Modbus::writeCoil(uint8_t index, CoilType value)
{
    // Check for valid index within coils array
    if (index < coils.size()) {
        coils[index] = value;
...
void Modbus::writeInputRegister(uint8_t index, InputRegisterType value)
{
    // Check for valid index within inputRegisters array
    if (index < inputRegisters.size()) {
...
void Modbus::writeMultipleCoils(uint8_t start_index, const CoilType values[], size_t num_values)
{
...
void Modbus::writeMultipleInputRegisters(uint8_t start_index, const InputRegisterType values[], size_t num_values)
{
    // Check for valid start index and values length
